...
/**
 * updates numerical values for last skyscraper reached in glass cockpit
 */
public class LastDisplayComponent extends Component {
     ...
     * @param gw
     */
    public LastDisplayComponent(GameWorld gw) {
        this.gw = gw;
        try {
        ...
        for(GameObject curObj : gw.gameObjectCollection) {
            if(curObj instanceof PlayerHelicopter) {
                clockDigits[0] = digitImages[((PlayerHelicopter) curObj).getLastSkyScraperReached()];
            }
        }

 ...
 */
public class HeadingDisplayComponent extends Component {
     ...
     * @param gw
     */
    public HeadingDisplayComponent(GameWorld gw) {
        this.gw = gw;
        try {
        ...
        for(GameObject curObj : gw.gameObjectCollection) {
            if(curObj instanceof PlayerHelicopter) {
                clockDigits[0] = digitImages[((PlayerHelicopter) curObj).getHeading() / 100];
                clockDigits[1] = digitImages[((PlayerHelicopter) curObj).getHeading() % 100 / 10];
                clockDigits[2] = digitImages[((PlayerHelicopter) curObj).getHeading() % 10];
            }
        }
